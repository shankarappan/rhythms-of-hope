...
import { MERCH_MAX_ITEMS, MERCH_PICKUP, MERCH_PRICE_CENTS, merchSizes, type MerchAudience, type MerchCartItem, type MerchColour, type MerchSize } from "../../lib/merch";
...
export function MerchShop() {
  ...
  const [audience, setAudience] = useState<MerchAudience>("adult");
  ...
  const add = () => {
    ...
    setCart(current => {
      const index = current.findIndex(item => item.colour === colour && item.audience === audience && item.size === size);
      if (index < 0) return [...current, { colour, audience, size, quantity }];
  ...
  return <section className="merch section" id="shop">
    <div className="merch__intro" data-reveal>
      ...
      <p>Choose a black or white HOPE T-shirt for adults or kids. Every purchase helps support the purpose of Rhythms of Hope and the community conversations it brings together.</p>
      <div className="merch__facts"><span>NZ$35 each</span><span>Adult + kids</span><span>Venue pickup</span></div>
    </div>
    <div className="merch-builder" data-reveal>
      ...
      <div className="merch-config">
        ...
        <fieldset><legend>Choose a fit</legend><div className="merch-options"><button className={audience === "adult" ? "is-active" : ""} onClick={() => setAudience("adult")}>Adult</button><button className={audience === "kids" ? "is-active" : ""} onClick={() => setAudience("kids")}>Kids</button></div></fieldset>
    ...
    <aside className="merch-cart" aria-live="polite">
      ...
      <div className="merch-cart__items">{cart.length ? cart.map((item, index) => <div key={`${item.audience}-${item.colour}-${item.size}`}><span>{item.audience === "adult" ? "Adult" : "Kids"} · {item.colour} · {item.size}</span><strong>× {item.quantity}</strong><button aria-label="Remove item" onClick={() => setCart(current => current.filter((_, itemIndex) => itemIndex !== index))}>Remove</button></div>) : <p>Select your options above, then add a shirt to begin.</p>}</div>
